...
Prop-table ownership decided 2026-08-31, after the same drift regenerated in the filter surfaces merge: the page for the thing owns the prop table, `components.md` lists it with a link, and `components.md` keeps a full entry only for what belongs to no single page - the root, the Table, the hooks.
The `## TMDataGrid.X` heading is the ownership marker, so `demos.test.tsx` fails when one appears on two pages; the rule is in CLAUDE.md.
The three filter components were the only pair doubled, and they moved to a link table.
The rest of `components.md` stands: no topic page writes those parts up yet, so nothing is duplicated. Moving them page by page is ordinary work, not a migration.
