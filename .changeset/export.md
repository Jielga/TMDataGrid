...
- `columns="custom"` on the menu items opens a column picker: every exportable column, the visible ones ticked, Export and Cancel. `ui.state.exportPicker`, `ui.actions.openExportPicker` / `closeExportPicker`, `getExportableColumns`.
...
- `data-dg-part`: `menu-export`, `menu-export-selected`, `export-picker`, `export-column`, `export-picker-confirm`, `export-picker-cancel`.
- Labels: `exportAll`, `exportSelected(count)`, `exportPickerTitle`, `exportPickerConfirm`, `exportPickerCancel`, `exportCells` (the cell-range item, was `exportCsv`).
