...
export type TMDataGridExportApi = {
  /** Downloads every filtered and sorted row, all pages. */
 ...
 * another format, another file name.
 */
export function useTMDataGridExport(
  ...
  const run = async (rows: "all" | "selected") => {
    const data = buildExportData({ table, rows });
    if (data.columnIds.length === 0) return;
    if (rows === "selected" && data.rows.length === 0) return;
    await writeExportFile(data, resolveExportOptions(exportOptions, overrides));
  };

  return {
    exportAll: () => run("all"),
    exportSelected: () => run("selected"),
    selectedCount,
